...
--4. EMP 테이블에서 부서 위치가 Dallas인 모든 종업원에 대해 
--이름,업무,급여를 출력하는 SELECT문을 작성하시오.
SELECT * FROM EMP;
SELECT * FROM DEPT;
...
AND SAL IN(SELECT SAL FROM EMP WHERE ENAME='FORD') AND ENAME<>'FORD';
...
SELECT * FROM EMP;
--13. EMP 테이블에서 부서별로 월급이 평균 월급보다 높은 사원을 부서번호,이름,급여를 출력하는 SELECT문을 성하시오.
SELECT E.DEPTNO, E.ENAME, E.SAL
FROM EMP E,
(SELECT DEPTNO,AVG(SAL) A_PAY FROM EMP GROUP BY DEPTNO) D
...
SELECT E.DEPTNO, E.ENAME, E.SAL
FROM EMP E,
(SELECT JOB,AVG(SAL) SAL_PAY FROM EMP GROUP BY JOB) D
WHERE E.JOB = D.JOB
AND E.SAL < D.SAL_PAY;


--15. EMP 테이블에서 적어도 한명 이상으로부터 보고를 받을 수 있는 사원을 업무,이름,사원번호,부서번호를 출력하는 SELECT문을 작성하시오.
SELECT DISTINCT B.JOB,B.ENAME,B.EMPNO,B.DEPTNO 
FROM EMP A, EMP B
WHERE A.MGR = B.EMPNO; 
